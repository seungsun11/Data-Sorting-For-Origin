...
def Data_Merge_file_G_V (original_file_path, merged_file_path):
    #region
        ...
        for row_idx, row in enumerate(ws.iter_rows(values_only=True), start=2):
            for col_idx, cell_value in enumerate(row, start=1):
                ...
                if col_idx == 2:
                    ...
                    if cell_value == '#REF':
                        merged_ws.cell(row=row_idx, column=column_offset + col_idx-1, value=0)
                    else:
                        merged_ws.cell(row=row_idx, column=column_offset + col_idx-1, value=cell_value)
        
        # 다음 시트 데이터를 위한 열 오프셋 조정
        column_offset += ws.max_column-1
